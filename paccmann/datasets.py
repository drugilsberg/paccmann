...
# TFRecords metadata.
DATASETS_METADATA = {
    'smiles_character_tokens': tf.FixedLenFeature(
        (158), tf.int64
    ),
    'smiles_atom_tokens': tf.FixedLenFeature(
        (155), tf.int64
    ),
    'fingerprints_256': tf.FixedLenFeature(
        (256), tf.int64
    ),
    'fingerprints_512': tf.FixedLenFeature(
        (512), tf.int64
    ),
    'targets_10': tf.FixedLenFeature(
        ((10, 1)), tf.float32
    ),
    'targets_20': tf.FixedLenFeature(
        ((20, 1)), tf.float32
    ),
    'targets_50': tf.FixedLenFeature(
        ((50, 1)), tf.float32
    ),
    'selected_genes_10': tf.FixedLenFeature(
        (1121), tf.float32
    ),
    'selected_genes_20': tf.FixedLenFeature(
        (2128), tf.float32
    ),
    'cnv_min': tf.FixedLenFeature(
        (2128), tf.int64
    ),
    'cnv_max': tf.FixedLenFeature(
        (2128), tf.int64
    ),
    'disrupt': tf.FixedLenFeature(
        (2128), tf.int64
    ),
    'zigosity': tf.FixedLenFeature(
        (2128), tf.int64
    ),
    'ic50': tf.FixedLenFeature(
        (), tf.float32
    ), 
    'ic50_labels': tf.FixedLenFeature(
        (), tf.int64
    ), 
}
...
def generate_dataset(
    filepath, buffer_size=int(256e+6),
    ...
    logger.debug('Parsing examples from the following files: {}'.format(
        filenames
    ))
...
def get_sorted_filelist(filepath, pattern='tfrecords'):
    ...
    return sorted([
            filename
            for filename in map(
                lambda entry: os.path.join(filepath, entry),
                os.listdir(filepath)
            )
